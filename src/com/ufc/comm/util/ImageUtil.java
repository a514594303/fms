...
public class ImageUtil {
	...
	private static BufferedImage getRoundedImage(BufferedImage img, int size,
		...
		g.drawImage(fixedImg, (size - fixedImg.getWidth(null)) / 2,
				(size - fixedImg.getHeight(null)) / 2, null);// 在适当的位置画出

		// 圆角
		if (radius > 0) {
			RoundRectangle2D round = new RoundRectangle2D.Double(0, 0, size,
	...
	protected static BufferedImage get24BitImage(BufferedImage $image) {
		...
		BufferedImage __newImg = new BufferedImage(__w, __h,
	...
	public static void toJPG(File img, File save, int size, int quality)
			throws IOException {
		FileOutputStream out = new FileOutputStream(save);
		JPEGImageEncoder encoder = JPEGCodec.createJPEGEncoder(out);

		BufferedImage image = (BufferedImage) getRoundedImage(
				ImageIO.read(img), size, 0, 2);// 默认无圆角
	...
	public static void toPNG(File img, File save) throws IOException {
		BufferedImage bi = ImageIO.read(img);
		ImageIO.write(
				(BufferedImage) getRoundedImage(bi, bi.getWidth(),
						bi.getWidth(), 2), "PNG", save);// 默认无圆角
	}

	public static void main(String[] args) throws IOException {
		File img = new File("g:/9.png");
		File save = new File("g:/10.png");
